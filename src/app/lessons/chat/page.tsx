...
export default async function ChatPage() {
  ...
  const [{ data: groupMessages }, { data: allPMs }] = await Promise.all([
    ...
    supabase.from('private_messages').select('id, content, created_at, sender_id, receiver_id, read_at, reply_to_id').or(`sender_id.eq.${user.id},receiver_id.eq.${user.id}`).order('created_at', { ascending: false }),
  ])
